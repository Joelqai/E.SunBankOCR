    ...
    def execute(self):
        ...
        for filename in filenames:
            try:
                ...
                print(filename,"out of class")

class ESunHandWritingPatchPreprocess(AllocationDataToDifferentFolder):
    ...
    def execute(self):
        ...
        for filename in filenames:
            try:
                if not os.path.isfile(os.path.join(os.path.join(self.destination_folder_path,vocab_dict.get(filename[0])),filename)):
                    src_complete_path = os.path.join(self.source_folder_path,filename)
                    dst_complete_path = os.path.join(self.destination_folder_path,vocab_dict.get(filename[0]))
                    shutil.copy(src=src_complete_path, dst=dst_complete_path)
                    print(f"add file {filename}")
            except:
                print(filename,"out of class")
